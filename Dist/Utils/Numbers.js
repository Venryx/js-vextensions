 ...
 */
export function Range(min, max, step = 1, includeMax = true, fixDecimalError_precision = 12) {
    var result = [];
    for (let i = min; includeMax ? i <= max : i < max; i = fixDecimalError_precision != null ? NumberCE(i + step).FixDecimalError(fixDecimalError_precision) : i + step) {
        result.push(i);
    }
    return result;
}
